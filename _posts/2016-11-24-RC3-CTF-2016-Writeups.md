...
Specifying ```..```, ```/```, ```;```, ```$()``` as a parameter value did not work, there must be logic in the application that sanitizes the input. However URL-encoding ```&&``` worked, parameter value ```* %26%26 ls -la``` got us the following list:
